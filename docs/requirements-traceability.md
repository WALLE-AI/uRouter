...
| Embeddable decision facade | Design milestone M4 | code_complete | `urouter-core` owns the zero-I/O Route decision API; Embed has explicit CapacitySnapshot injection, host Driver, two runnable examples, and same-artifact Gateway-Core parity without gateway/state/client dependencies. The published surface now packages: every intra-workspace dependency carries a version requirement and non-published crates are marked `publish = false`, so `cargo package` no longer fails unconditionally | crate publication and downstream adoption |
| Structured logging | Design section 18 | code_complete | `crates/urouter-gateway/src/logging.rs`; a `chat` span carries request/decision/tenant/tier/model, every error response emits one line with its stable code, and bodies are never fields; `--log-format json\|text` with `RUST_LOG` override | log backend ingestion and retention review |
| Container and orchestrator delivery | Design section 15 | code_complete | distroless `Dockerfile`, two-Gateway-plus-Redis `docker-compose.yml`, `deploy/kubernetes/gateway.yaml` with drain-consistent probe and grace timings; `urouter-xtask check-deploy` validates every shipped argument set against the Gateway dry-run | image build, registry publication and a real rollout drill |
| Supply-chain policy | Design section 15 | code_complete | `deny.toml` gates advisories, licenses, banned packages and source registries; `onnx-pb` is confined by a wrapper ban so RUSTSEC-2021-0073 cannot reach the Gateway tree | periodic advisory review |
